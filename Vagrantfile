...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "dn0", primary: true do |dn0|
    ...
    dn0.vm.network :forwarded_port, guest: 22, host: 22, id: "ssh"
    # MySQL Server
    dn0.vm.network(:forwarded_port, {:guest=>3306, :host=>3306})
    # karamel http
    dn0.vm.network(:forwarded_port, {:guest=>9090, :host=>9090})     
    # Hopsworks http
    dn0.vm.network(:forwarded_port, {:guest=>8080, :host=>8080})     
    # Glassfish debug port
    dn0.vm.network(:forwarded_port, {:guest=>9009, :host=>9009})
    # Glassfish admin UI
